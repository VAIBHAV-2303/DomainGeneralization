...
def get_train_dataloader(args, patches):
    ...
    for dname in dataset_list:
        ...
        train_dataset = JigsawNewDataset(args.root_dir, name_train, labels_train, patches=patches, img_transformer=img_transformer,
                                      tile_transformer=tile_transformer, jig_classes=30, bias_whole_image=args.bias_whole_image)
        if limit:
            train_dataset = Subset(train_dataset, limit)
        datasets.append(train_dataset)
        val_datasets.append(
            JigsawTestNewDataset(args.root_dir, name_val, labels_val, img_transformer=get_val_transformer(args),
...
def get_val_dataloader(args, patches=False):
    ...
    val_dataset = JigsawTestNewDataset(args.root_dir, names, labels, patches=patches, img_transformer=img_tr, jig_classes=30)
