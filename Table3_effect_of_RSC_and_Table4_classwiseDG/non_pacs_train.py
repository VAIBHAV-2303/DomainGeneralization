...
from data import non_pacs_data_helper
## from IPython.core.debugger import set_trace
from data.non_pacs_data_helper import available_datasets
from models import model_factory
from optimizer.optimizer_helper import get_optim_and_scheduler
from utils.non_pacs_Logger import Logger
import numpy as np
...
def get_args():
	...
	parser.add_argument("--root_dir", type=str, default="./data/images", help="Root Directory for PACS Dataset")
...
class Trainer:
	def __init__(self, args, device):
		...
		if args.network == 'resnet18':
			...
			model = MyInceptionResnet()
		else:
			model = resnet18(pretrained=True, classes=args.n_classes)
		self.model = model.to(device)
		# print(self.model)
		self.source_loader, self.val_loader = non_pacs_data_helper.get_train_dataloader(args, patches=model.is_patch_based())
		self.target_loader = non_pacs_data_helper.get_val_dataloader(args, patches=model.is_patch_based())
	...
	def _do_epoch(self, epoch=None):
		...
		for it, batch in enumerate(self.source_loader):
			data, class_l = batch[0].to(self.device), batch[1].to(self.device)
