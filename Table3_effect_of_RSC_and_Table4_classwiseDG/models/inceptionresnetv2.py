...
class InceptionResNetV2(nn.Module):
	...
	def forward(self, input, gt=None, flag=None, epoch=None):
...
class MyInceptionResnet(torch.nn.Module):
	...
	def __init__(self, num_classes):
		...
		self.pretrained = inceptionresnetv2(pretrained='imagenet')
		...
		self.class_classifier = torch.nn.Linear(1536, num_classes)
		self.pecent = 1/3
	...
	def forward(self, x, gt=None, flag=None, epoch=None):
		...
		if flag:
			...
			if choose_one <= 4:
				...
				th18_mask_value = th18_mask_value.view(num_rois, 1).expand(num_rois, 25)
